                                        ...
                                        mbDeactivateLocalizationMode(false)
{
    ...
    if (mSensor != STEREO)
    {
        throw std::runtime_error("Only stereo system is allowed");
    }
...
cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const cv::Mat &rotDiff)
{
    if (mSensor != STEREO)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageStereo(imLeft, imRight, timestamp, rotDiff);
